...
```{r}
#| label: load-data
surveys <- read_csv("data/surveys.csv") # reads csv file data
glimpse(surveys) # loads data frame summary
```

**1. What are the dimensions (# of rows and columns) of these data?**

> There are 30463 rows and 15 columns on this dataset. 

**2. What are the data types of the variables in this dataset?**

> Variables such as sex, species, and taxa represent qualitative nominal variables. Variables such as weight and hindfoot length represent quantitave continuous variables. Variables such as month, day, and year, represent quantitave discrete variables. 
...
```{r}
#| label: scatterplot
ggplot(data = surveys) + # creates scatterplot
  (mapping = aes(x = weight, y = hindfoot_length, color = species, fill = species)) +
  geom_point(alpha = 10) # adds transparency to data points +
  labs(
    title = "Weight and Hindfoot Length Among Rodent Species",
    x = "Weight (g)",
